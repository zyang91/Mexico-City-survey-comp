...
metro_2007<- metro %>%
  filter(!is.na(count2)) %>%
  mutate(percet_metro_2007 = count2/sum(count2))

maxy3 <- max(metro_2007$percet_metro_2007, na.rm = TRUE)

ggplot(metro_2007,
       aes(x = reorder(multi_label, percet_metro_2007),
           y = percet_metro_2007,
           fill = multi_label)) +
  geom_col() +
  geom_text(aes(label = scales::percent(percet_metro_2007, accuracy = 0.1)),
...
non_brt_2017<- tables %>%
  filter(x== "Non BRT bus") %>%
  filter(!is.na(count_2)) %>%
  mutate(percet_non_brt_2017 = count_2/sum(count_2))
maxy8 <- max(non_brt_2017$percet_non_brt_2017, na.rm = TRUE)
ggplot(non_brt_2017,
       aes(x = reorder(multi_la_17, percet_non_brt_2017),
           y = percet_non_brt_2017,
           fill = multi_label)) +
  geom_col() +
  geom_text(aes(label = scales::percent(percet_non_brt_2017, accuracy = 0.1)),
            hjust = -0.1, size = 2.8) +  # smaller text
  coord_flip(clip = "off") +
  scale_y_continuous(
    labels = scales::percent_format(),          # assumes data are 0–1
